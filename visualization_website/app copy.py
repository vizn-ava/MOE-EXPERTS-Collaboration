...
@app.route('/api/expert-token-mapping', methods=['GET'])
def get_expert_token_mapping():
    """获取专家-token映射关系"""
    try:
        ...
        return jsonify({'error': f'读取映射文件失败: {str(e)}'}), 500


@app.route('/api/expert-token-mapping', methods=['POST'])
def save_expert_token_mapping():
    """保存专家-token映射关系"""
    try:
        data = request.get_json(silent=True) or {}
        mapping_path = os.path.join(os.path.dirname(__file__), 'expert_token_mapping.json')
        ...
        return jsonify({'success': True, 'message': '映射关系保存成功'})
    except Exception as e:
        return jsonify({'error': f'保存映射文件失败: {str(e)}'}), 500
