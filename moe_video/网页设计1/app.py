...
USE_REMOTE = os.environ.get('USE_REMOTE', 'true').lower() == 'true'
# 复用 HTTP 连接，减少握手时延
...
def apply_top_k(vector: List[float], k: int = 6) -> List[float]:
    """应用Top-K选择，保留前k个最大的值，其他设为0"""
    if len(vector) <= k:
        return vector
    ...
    top_k_indices = {i for i, _ in indexed_values[:k]}
